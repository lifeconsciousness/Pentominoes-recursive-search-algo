...
public class Search {
    ...
    /**
     * this is a brute force algorithm that goes through all the Pentominoes and its 
     * permutations for every x and y coordinates on the field
     * @param field
     * @param check
     * @param depths
     */
    public static void bruteForce(int[][] field, int[][] check, int depths){
        boolean solutionFound = false;
        boolean stopAttempt = false;
        long solutionCounter = 0;
        ui.setState(field);
        
        // goes through all Pentominoes
        for(int i = 0; i < input.length; i++){
            int pentId = characterToID(input[i]);
            
            // goes through all permutations for that Pentominoe
            for(int j = 0; j < PentominoDatabase.data[pentId].length; i++) {
                ...
                for(int k = 0; k < pieceToPlace.length; k++){ 
                    ...
                    for(int x = 0; x < horizontalGridSize; x++){
                        for(int y = 0; y < verticalGridSize; y++){
                            ...
                            // check if Field is Empty and Current Permutation fits
        ...
        /**
         * Checks if Pentominoe fits into Field at given Coordinates if yes runs addPiece if no  changes stopAttempt to true and solutionFound to false
         * @param field
         * @param piece
         * @param pieceId
         * @param x
         * @param y
         */
    public static void pieceFits(int[][] field, int[][] piece, int pieceId, int x, int y) {
        for(int xField = x; x < piece.length; x++) {
            for(int yField = y; y < piece[0].length; y++) {
